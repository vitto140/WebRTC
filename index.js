...
app.post('/send-email', async (req, res) => {
    ...
    console.log('Flowers:', req.body.flowers);
    console.log('Has voice note:', !!req.body.voiceNote);

    try {
        const { recipient, message, flowers, voiceNote } = req.body;

        if (!recipient) {
            return res.status(400).json({ success: false, error: 'No recipient provided' });
        }
        ...
        // Convert voice note (SKIP if no voice note to avoid crash)
        let voiceHTML = '';
        if (voiceNote && voiceNote.length > 0) {
            console.log('🎤 Converting voice note...');
            try {
                ...
                voiceHTML = `<div class="voice-badge">🎤 Voice Message Attached</div>`;
                console.log('✅ Voice note converted');
            } catch (voiceError) {
                console.error('❌ Voice conversion failed:', voiceError);
